...
function GridironGame() {
  ...
  function beginRandomGame(away,home,playerSide,aC,hC,quarters) {
    // Build two full random rosters directly — no draft screen
    const pool=buildDraftPool();
    ...
    setG(prev=>({
      ...prev,
      teams:{away,home}, playerSide, teamColors:{away:aC,home:hC}, quarterChoice:quarters,
      phase:"coin_toss",
      draftPool:pool,
  ...
  function advanceGauntletLevel(chosenPlayer=null) {
    ...
    // Apply upgrade: swap in chosen player
    let roster = {...gauntlet.playerRoster};
    if(chosenPlayer) {
      const starters = [...roster.starters];
      // Find same position to replace — prefer lowest-rated of same type
      const samePos = starters
        .map((p,i)=>({p,i}))
        .filter(({p})=>p.pos===chosenPlayer.pos||p.side===chosenPlayer.side);
      if(samePos.length>0) {
        const overall = p=>p.side==="offense"?(p.primary+p.secondary+p.speed+p.awareness)/4:(p.pass_rush+p.coverage+p.tackle+p.instinct)/4;
        const worst = samePos.sort((a,b)=>overall(a.p)-overall(b.p))[0];
  ...
  // Generate 3 upgrade options — always better than current roster
  function getUpgradeOptions(roster, level) {
    ...
    const avail = pool
      ...
      .filter(p=>p.side==="offense") // gauntlet upgrades: offense only
      .filter(p=>overall(p)>55+level*2)
    ...
    setG(prev=>{
      ...
      if(next>=draftLimit) {
        ...
        const filledPlayer = isGauntletDraft
          ? {starters:autoFillRoster(newStarters,prev.draftPool,allTaken,FULL_OFFENSE,0),bench:[]}
  ...
  if(gauntletPhase==="gameover"&&gauntlet) {
    ...
    return (
      <div style={root}>
        <div style={{...card,maxWidth:440,margin:"40px auto 0",textAlign:"center"}}>
      ...
      <div style={root}>
        <div style={{width:"100%",maxWidth:580,padding:"14px 16px 0",boxSizing:"border-box"}}>
          ...
          <div style={{marginTop:14,marginBottom:8}}>
            ...
            <div style={{fontSize:12,color:"#64748b",marginBottom:10}}>Pick a player — replaces the weakest at their position. Or skip.</div>
  ...
  if(gauntletPhase==="draft"&&g.phase!=="draft") {
    return (
      <div style={root}>
        <div style={{...card,maxWidth:400,margin:"40px auto 0",textAlign:"center"}}>
          ...
          <div style={{textAlign:"left",marginBottom:12}}>
            ...
            <div style={{position:"relative"}}>
              <select value={teamName} onChange={e=>setTeamName(e.target.value)}
                style={{width:"100%",background:"#1e293b",border:"1px solid #334155",color:"#f8fafc",borderRadius:8,padding:"10px 14px",fontSize:14,boxSizing:"border-box",appearance:"none",cursor:"pointer",outline:"none"}}>
          ...
          <button style={btn("#f59e0b")} onClick={()=>{
            if(skipDraft) {
              // Build a random offense roster and launch directly
              const pool=buildDraftPool();
              const offPlayers=pool.filter(p=>p.side==="offense").sort(()=>Math.random()-0.5).slice(0,FULL_OFFENSE);
              const roster={starters:offPlayers,bench:[]};
        ...
        <div style={{...card,maxWidth:420,margin:"40px auto 0"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"stretch"}}>
            ...
            <button style={{...btn(TEAM_PALETTE[playerColorIdx].primary),flex:1}} onClick={()=>{
              ...
              if(skipDraft) {
                beginRandomGame(teamName,aiTeamName||"Opponents","away",pC,aC2,quarterChoice);
              } else {
          ...
          {showControls ? (
            <>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                ...
                {/* Punt always available on 4th down */}
                {g.down===4&&(
                  <button onClick={()=>choosePitch("punt")}
                    style={{...offBtn("#475569",false),display:"flex",justifyContent:"space-between",alignItems:"center",borderColor:"#334155",padding:"10px 12px"}}>
                    <span style={{fontSize:16,marginRight:6}}>👢</span>
                    <span style={{flex:1,textAlign:"left",fontWeight:800}}>Punt</span>
                    <span style={{fontSize:11,color:"#64748b",fontWeight:400}}>Kick it away</span>
                  </button>
                )}
                {/* FG available when in range */}
                {yardsToTD(g.fieldPosition)<=45&&(
                  <button onClick={()=>choosePitch("field_goal")}
                    style={{...offBtn("#f59e0b",false),display:"flex",justifyContent:"space-between",alignItems:"center",borderColor:"#92400e",padding:"10px 12px"}}>
                    <span style={{fontSize:16,marginRight:6}}>⚡</span>
                    <span style={{flex:1,textAlign:"left",fontWeight:800}}>Field Goal</span>
                    <span style={{fontSize:11,color:"#92400e",fontWeight:700}}>{yardsToTD(g.fieldPosition)+17} yd attempt</span>
                  </button>
                )}
              </div>
            </>
  ...
  if(g.phase==="play_call_defense") {
    ...
    const showControls = isGauntlet ? false : isOnline ? showDefenseControls : (g.mode==="two-player" || playerOnDef);
          ...
          {onlineState==="idle"&&(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <button style={btn("#E63946")} onClick={async()=>{
                ...
                await supa.upsertRoom(code,{status:"waiting",quarters:quarterChoice,skipDraft,awayName:teamName,awayColorIdx:playerColorIdx});
              }}>🏠 Create Room</button>
            ...
            <div>
              ...
              <button style={btn("#E63946")} onClick={()=>{
                ...
                if(skipDraft) {
                  ...
                  const takenIds=playerRoster.starters.map(p=>p.id);
                  const aiRoster=buildAIRoster(pool,takenIds);
                  setG({...initGame(),mode:"online",phase:"coin_toss",_playoffDraft:false,playerSide:"away",_turn:"home",playerRoster,aiRoster,teams:{away:teamName,home:"Opponent"},teamColors:{away:TEAM_PALETTE[playerColorIdx],home:TEAM_PALETTE[(playerColorIdx+3)%10]},quarterChoice,gameLog:[`🏈 ${teamName} vs Opponent (Random Teams)`]});
                } else {
