...
function GridironGame() {
  ...
  function getUpgradeOptions(roster, level) {
    ...
    const overall = p=>(p.primary+p.secondary+p.speed+p.awareness)/4;
    // Count current tiers in roster (excluding C which are fillers)
    const tierCount = {S:0,A:0,B:0};
    roster.starters.forEach(p=>{ if(tierCount[p.tier]!==undefined) tierCount[p.tier]++; });
    // Only offer tiers that still have room under gauntlet limits
    const avail = pool
      .filter(p=>!takenIds.includes(p.id))
      .filter(p=>p.side==="offense")
      .filter(p=>{
        // C-tier fillers can always be upgraded to any tier with room
        const limit = GAUNTLET_TIER_LIMITS[p.tier] ?? 0;
        if(limit===0) return false; // never offer C-tier as upgrade
        return (tierCount[p.tier]||0) < limit;
      })
        ...
        <div style={card}>
          ...
          {isGauntlet&&(
            <div style={{marginBottom:8}}>
              <button onClick={()=>setShowGauntletRoster(s=>!s)}
                style={{background:"#0f172a",border:`1px solid ${showGauntletRoster?playerColor:"#1e293b"}`,color:showGauntletRoster?playerColor:"#475569",borderRadius:8,padding:"6px 12px",fontSize:11,fontWeight:700,cursor:"pointer",width:"100%"}}>
