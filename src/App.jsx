...
function GridironGame() {
  ...
  // Push state to Supabase when it's our turn
  const lastPushedRef = useRef(null);
  useEffect(()=>{
    if(g.mode!=="online"||!roomCode||onlineState!=="playing") return;
    if(!g._turn||g._turn!==onlineSide) return;
    const key=`${g.phase}|${g._turn}|${g.down}|${g.score?.away}|${g.score?.home}|${g.possession}|${g.offensePlay||""}|${g.defensePlay||""}`;
  ...
  function choosePitch(playId) {
    // Offense calls play → _turn flips to defense side to call their scheme
    const defSide = g.possession==="away" ? "home" : "away";
    setG(prev=>({...prev, offensePlay:playId, phase:"play_call_defense", _turn:defSide}));
  }

  function chooseDefensePlay(playId) {
    // Defense calls scheme → resolve dice → _turn goes to offense to see result and continue
    setG(prev=>({
      ...prev, defensePlay:playId,
      lastDice: resolvePlay(
      ...
      phase:"rolling",
      _turn: prev.possession, // offense's turn to roll through and see result
    }));
  }
