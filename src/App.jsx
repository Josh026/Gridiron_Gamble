...
// ─── SCOREBOARD ───────────────────────────────────────────────────────────────
function Scoreboard({g, onSoundToggle}) {
    ...
    <div style={{width:"100%",maxWidth:580,padding:"8px 10px 0",boxSizing:"border-box"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr auto 1fr",alignItems:"center",gap:8,marginBottom:6}}>
        <div style={{background:hasBall==="away"?aC.primary+"22":"#111827",border:`2px solid ${hasBall==="away"?aC.primary:"#1e293b"}`,borderRadius:10,padding:"6px 10px",textAlign:"center"}}>
          <div style={{fontSize:9,color:"#64748b",textTransform:"uppercase",letterSpacing:1.5,marginBottom:2}}>{g.teams.away}{hasBall==="away"?" 🏈":""}</div>
        ...
        <div style={{background:hasBall==="home"?hC.primary+"22":"#111827",border:`2px solid ${hasBall==="home"?hC.primary:"#1e293b"}`,borderRadius:10,padding:"6px 10px",textAlign:"center"}}>
          <div style={{fontSize:9,color:"#64748b",textTransform:"uppercase",letterSpacing:1.5,marginBottom:2}}>{g.teams.home}{hasBall==="home"?" 🏈":""}</div>
...
function GridironGame() {
  ...
  useEffect(()=>{
    ...
    const playerOnDef = g.possession!==g.playerSide;

    // AI offense: auto pick play when AI has ball and we need offense call
    if(g.phase==="play_call_offense" && !playerHasBall) {
      const lastOffPlay = g.driveLog.slice(-1)[0]?.offensePlay || null;
    ...
    // AI defense: auto pick play when AI is defending
    if(g.phase==="play_call_defense" && playerHasBall) {
      const lastOffPlay = g.offensePlay;
      const play = aiPickDefensePlay(lastOffPlay, g.down, g.yardsToGo, g.score, g.possession==="away"?"home":"away", g.quarter);
  ...
  // Auto-advance result after 5s in vs-ai — but NOT when waiting for special teams choice
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    const poll=setInterval(async()=>{
      ...
      if(remote._writer===onlineSide) return;
      setG(prev=>({...remote,playerSide:prev.playerSide,playerRoster:prev.playerRoster,aiRoster:prev.aiRoster,mode:"online",playGuess:prev.playGuess}));
    },1500);
    return()=>clearInterval(poll);
  },[onlineState,roomCode,onlineSide]);

  // Push online state when it's our turn
  const lastPushedRef = useRef(null);
  useEffect(()=>{
    if(g.mode!=="online"||!roomCode||onlineState!=="playing") return;
    if(!g._turn||g._turn!==onlineSide) return;
    const key=g.phase+g._turn+(g.down||0)+(g.score?.away||0)+(g.score?.home||0);
    if(lastPushedRef.current===key) return;
    lastPushedRef.current=key;
    supa.upsertRoom(roomCode,{...g,_writer:onlineSide}).catch(()=>{});
  ...
  function nextPlayState(prev) {
    // Called after a play result is acknowledged — determine what happens next
    return {...prev, phase:"play_call_offense", offensePlay:null, defensePlay:null, playGuess:null, lastResult:null};
  }
  ...
  function choosePitch(playId) {
    // Offense calls their play
    setG(prev=>({...prev,offensePlay:playId,phase:"play_call_defense",_turn:prev.possession==="away"?"home":"away"}));
  }

  function chooseDefensePlay(playId) {
    setG(prev=>({
      ...prev,defensePlay:playId,
      lastDice:resolvePlay(prev.offensePlay,playId,
        prev.possession===prev.playerSide?prev.playerRoster:prev.aiRoster,
        prev.possession===prev.playerSide?prev.aiRoster:prev.playerRoster,
        prev.playGuess,{yardsToTD:yardsToTD(prev.fieldPosition)}),
      phase:"rolling",_turn:prev.possession,
    }));
  }
    ...
    return (
      <div style={root}>
        <div style={{...card,maxWidth:380,margin:"80px auto 0",textAlign:"center"}}>
          ...
          <div style={{fontSize:13,color:"#64748b",marginBottom:14}}>You won the toss — choose:</div>
          <div style={{display:"flex",gap:10}}>
            <button style={{...btn(playerColor),flex:1}} onClick={()=>{
              // Player receives: opponent kicks, so pass opponent's side to kickoff()
              setG(prev=>{ const opp=prev.playerSide==="away"?"home":"away"; return {...prev,...kickoff(opp),teams:prev.teams,teamColors:prev.teamColors}; });
            }}>🏈 Receive</button>
            <button style={{flex:1,background:"#1e293b",border:"1px solid #334155",color:"#94a3b8",borderRadius:10,padding:"12px 20px",fontWeight:800,fontSize:14,cursor:"pointer"}} onClick={()=>{
              // Player kicks: player's side kicks, so pass playerSide to kickoff()
              setG(prev=>({...prev,...kickoff(prev.playerSide),teams:prev.teams,teamColors:prev.teamColors}));
            }}>🦵 Kick</button>
          </div>
        </div>
      </div>
  ...
  if(g.phase==="play_call_offense") {
    const showPlayerControls = g.mode==="two-player"||playerHasBall;
    const offColor=g.teamColors[g.possession]?.primary||playerColor;
    const qb=g.possession===playerSide?g.playerRoster.starters.find(p=>p.pos==="QB"):g.aiRoster.starters.find(p=>p.pos==="QB");
    const rb=(g.possession===playerSide?g.playerRoster:g.aiRoster).starters.filter(p=>p.pos==="RB")[0];
    ...
    return (
      <div style={root}>
        <Scoreboard g={g} onSoundToggle={soundOn?{on:true,fn:()=>{_soundEnabled=false;setSoundOn(false);}}:{on:false,fn:()=>{_soundEnabled=true;setSoundOn(true);}}}/>
        <div style={card}>
          {/* Last play recap */}
          ...
          {showPlayerControls ? (
            ...
            <div style={{color:"#64748b",textAlign:"center",padding:"16px 0"}}>🤖 AI offense is huddle…</div>
          )}
          {/* Game log */}
  ...
  if(g.phase==="play_call_defense") {
    const showPlayerControls = g.mode==="two-player"||playerOnDef;
    const defSide=g.possession==="away"?"home":"away";
    const defColor=g.teamColors[defSide]?.primary||"#3b82f6";
    const defRoster=defSide===playerSide?g.playerRoster:g.aiRoster;
      ...
      <div style={root}>
        ...
        <div style={card}>
          ...
          {showPlayerControls ? (
            <>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:8}}>
                {DEFENSE_PLAYS.map(play=>{
                  // Only highlight scheme matchups in two-player (would reveal offense play otherwise)
            ...
            <div style={{color:"#64748b",textAlign:"center",padding:"16px 0"}}>🤖 Defense is reading the offense…</div>
          )}
        </div>
      </div>
    );
    ...
    return (
      <div style={root}>
        ...
        <div style={card}>
          ...
          <button style={btn(playerColor)} onClick={()=>setG(prev=>nextPlayState(prev))}>
            Next Play ▶{g.mode==="vs-ai"?" (or wait 2.5s)":""}
          </button>
        </div>
      </div>
    );
